...
ITextRange
IGlyphRun::GetCharacterRange() const
{
    TextRangeRef ret = 0;
    ATEErr error = sGlyphRun->GetCharacterRange(fGlyphRun, &ret);
    if(error) Throw_ATE_Exception(error);
...
ATETextDOM::Real IGlyphRun::GetXHeight() const
{
    ATETextDOM::Real ret;
    ATETextDOM::Float tempRet;
    ATEErr error = sGlyphRun->GetMinCapHeight(fGlyphRun, &tempRet);
...
IListStyle::IListStyle()
:fListStyle(0)
{
    ATEErr error = sListStyle->Initialize(&fListStyle);
    if(error) Throw_ATE_Exception(error);
}

IListStyle::IListStyle(ListStyleRef listStyle)
:fListStyle(listStyle)
{
}

IListStyle::IListStyle(const IListStyle& src)
:fListStyle(src.fListStyle)
{
    sListStyle->AddRef(fListStyle);
}

IListStyle::~IListStyle()
{
    sListStyle->Release(fListStyle);
}

IListStyle& IListStyle::operator=(const IListStyle& rhs)
{
    if (&rhs != this)
    {
        sListStyle->Release(fListStyle);
        fListStyle = rhs.fListStyle;
        sListStyle->AddRef(fListStyle);
    }
    return *this;
}

bool IListStyle::operator==(const IListStyle& rhs) const
{
    ATEBool8 ret = false;
    ATEErr error = sListStyle->IsEqual(fListStyle, rhs.fListStyle, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}
...
ListStyleRef IListStyle::GetRef() const
{
    return fListStyle;
}

bool IListStyle::IsNull() const
{
    return sListStyle->IsNull(fListStyle);
}

ATETextDOM::Int32 IListStyle::GetName( ATETextDOM::Unicode * name, ATETextDOM::Int32 maxLength) const
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyle->GetName(fListStyle, name, maxLength, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyle::GetNameSize( ) const
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyle->GetNameSize(fListStyle, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyle::GetNumberOfLevelStyles( ) const
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyle->GetNumberOfLevelStyles(fListStyle, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}


void IListStyle::GetNthTierIndents(ATETextDOM::Int32 index,
                                   ATETextDOM::Float   emWidth,
                                   ATETextDOM::Float * listIndent,
                                   ATETextDOM::Float * textIndent,
                                   ATETextDOM::Float * labelIndent)
{
    ATEErr error = sListStyle->GetNthTierIndents(fListStyle, index, emWidth, listIndent , textIndent, labelIndent);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::GetNthLevelTextIndent(ATETextDOM::Int32 index,
                                   ATETextDOM::Float * textIndent)
{
    ATEErr error = sListStyle->GetNthLevelTextIndent(fListStyle, index, textIndent);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::GetNthLevelLabelIndent(ATETextDOM::Int32 index,
                                   ATETextDOM::Float * labelIndent)
{
    ATEErr error = sListStyle->GetNthLevelLabelIndent(fListStyle, index, labelIndent);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::GetNthLevelLabelAlignment(ATETextDOM::Int32   index,
                                           ATE::BNListAlignment& labelAlignment)
{
    ATEErr error = sListStyle->GetNthLevelLabelAlignment(fListStyle, index, labelAlignment );
    if(error) Throw_ATE_Exception(error);
}

IFont IListStyle::GetNthLevelLabelFont( ATETextDOM::Int32 index, bool& useOriginalFont)
{
    FontRef ret = 0;
    ATEErr error = sListStyle->GetNthLevelLabelFont(fListStyle, index, &ret, useOriginalFont);
    if(error) Throw_ATE_Exception(error);
    return IFont(ret);
}

void IListStyle::GetNthLevelSequenceTag(ATETextDOM::Int32   index,
                                           ATE::ListStylePredefinedTag& sequenceGeneratorType)
{
    ATEErr error = sListStyle->GetNthLevelSequenceTag(fListStyle, index, sequenceGeneratorType);
    if(error) Throw_ATE_Exception(error);
}

ATETextDOM::Int32 IListStyle::GetNthLevelSequencePrefix(ATETextDOM::Int32   index,
                                            ATETextDOM::Unicode* prefix,
                                            ATETextDOM::Int32 maxLength)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyle->GetNthLevelSequencePrefix(fListStyle, index, prefix, maxLength, &ret );
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyle::GetNthLevelSequencePostfix(ATETextDOM::Int32   index,
                                            ATETextDOM::Unicode* postfix,
                                            ATETextDOM::Int32 maxLength)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyle->GetNthLevelSequencePostfix(fListStyle, index, postfix, maxLength, &ret );
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyle::GetNthLevelSequenceBullet(ATETextDOM::Int32   index,
                                            ATETextDOM::Unicode* bullet,
                                            ATETextDOM::Int32 maxLength)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyle->GetNthLevelSequenceBullet(fListStyle, index, bullet, maxLength, &ret );
    if(error) Throw_ATE_Exception(error);
    return ret;
}

void IListStyle::SetName( const ATETextDOM::Unicode * name)
{
    ATEErr error = sListStyle->SetName(fListStyle, name);
    if(error) Throw_ATE_Exception(error);
}

bool IListStyle::MatchesPredefinedResourceTag( ATE::ListStylePredefinedTag tag) const
{
    ATEBool8 ret;
    ATEErr error = sListStyle->MatchesPredefinedResourceTag(fListStyle, tag, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATE::ListStylePredefinedTag IListStyle::GetPredefinedResourceTag( ) const
{
    ATE::ListStylePredefinedTag ret;
    ATEErr error = sListStyle->GetPredefinedResourceTag(fListStyle, ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

bool IListStyle::IsEquivalent( IListStyle rhsListStyle) const
{
    ATEBool8 ret;
    ATEErr error = sListStyle->IsEquivalent(fListStyle, rhsListStyle.GetRef(), &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

bool IListStyle::IsModified( ) const
{
    ATEBool8 ret;
    ATEErr error = sListStyle->IsModified(fListStyle, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

bool IListStyle::IsPredefined( ) const
{
    ATEBool8 ret;
    ATEErr error = sListStyle->IsPredefined(fListStyle, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

void IListStyle::SetNthLevelIndentUnits( ATETextDOM::Int32 index, ATE::IndentUnits   units)
{
    ATEErr error = sListStyle->SetNthLevelIndentUnits(fListStyle, index, units);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::SetNthLevelTextIndent( ATETextDOM::Int32 index, ATETextDOM::Float indent)
{
    ATEErr error = sListStyle->SetNthLevelTextIndent(fListStyle, index, indent);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::SetNthLevelLabelIndent( ATETextDOM::Int32 index, ATETextDOM::Float indent)
{
    ATEErr error = sListStyle->SetNthLevelLabelIndent(fListStyle, index, indent);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::SetNthLevelLabelAlignment( ATETextDOM::Int32 index, ATE::BNListAlignment labelAlignment)
{
    ATEErr error = sListStyle->SetNthLevelLabelAlignment(fListStyle, index, labelAlignment);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::SetNthLevelLabelFont(ATETextDOM::Int32 index, IFont font, bool useOriginalFont)
{
    ATEErr error = sListStyle->SetNthLevelLabelFont(fListStyle, index, font.GetRef(), useOriginalFont);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::SetNthLevelSequenceGenerator(ATETextDOM::Int32 index,
                                              ATE::ListStylePredefinedTag sequenceGeneratorTag,
                                              const ATETextDOM::Unicode* prefix,
                                              const ATETextDOM::Unicode* postfix,
                                              const ATETextDOM::Unicode* bullet)
{
    ATEErr error = sListStyle->SetNthLevelSequenceGenerator(fListStyle, index, sequenceGeneratorTag, prefix, postfix, bullet);
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::InsertLevelAt( ATETextDOM::Int32              index,
                                ATE::IndentUnits               units,
                                ATETextDOM::Float              textIndent,
                                ATETextDOM::Float              labelIndent,
                                ATE::BNListAlignment           labelAlignment,
                                IFont                          font,
                                ...
                                const ATETextDOM::Unicode*     bullet)
{
    ATEErr error = sListStyle->InsertLevelAt(fListStyle, index, units, textIndent, labelIndent, labelAlignment, font.GetRef(), useOriginalFont, sequenceGeneratorTag, prefix, postfix, bullet );
    if(error) Throw_ATE_Exception(error);
}

void IListStyle::EraseLevel( ListStyleRef listStyle, ATETextDOM::Int32 index)
{
    ATEErr error = sListStyle->EraseLevel(fListStyle, index);
    if(error) Throw_ATE_Exception(error);
}

IListStyle IListStyle::Duplicate( ) const
{
    ListStyleRef ret = 0;
    ATEErr error = sListStyle->Duplicate(fListStyle, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}
...
IListStyleSet::IListStyleSet()
:fListStyleSet(0)
{
}

IListStyleSet::IListStyleSet(ListStyleSetRef listStyleset)
:fListStyleSet(listStyleset)
{
}

IListStyleSet::IListStyleSet(const IListStyleSet& src)
:fListStyleSet(src.fListStyleSet)
{
    sListStyleSet->AddRef(fListStyleSet);
}

IListStyleSet::~IListStyleSet()
{
    sListStyleSet->Release(fListStyleSet);
}

IListStyleSet& IListStyleSet::operator=(const IListStyleSet& rhs)
{
    if (&rhs != this)
    {
        sListStyleSet->Release(fListStyleSet);
        fListStyleSet = rhs.fListStyleSet;
        sListStyleSet->AddRef(fListStyleSet);
    }
    return *this;
}

bool IListStyleSet::operator==(const IListStyleSet& rhs) const
{
    return fListStyleSet == rhs.fListStyleSet;
}
...
ListStyleSetRef IListStyleSet::GetRef() const
{
    return fListStyleSet;
}

bool IListStyleSet::IsNull() const
{
    return sListStyleSet->IsNull(fListStyleSet);
}

ATETextDOM::Int32 IListStyleSet::GetSize( )
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyleSet->GetSize(fListStyleSet, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

IListStyle IListStyleSet::GetFirst( )
{
    ListStyleRef ret = 0;
    ATEErr error = sListStyleSet->GetFirst(fListStyleSet, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}

IListStyle IListStyleSet::GetLast( )
{
    ListStyleRef ret = 0;
    ATEErr error = sListStyleSet->GetLast(fListStyleSet, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}

IListStyle IListStyleSet::Item( ATETextDOM::Int32 nIndex) const
{
    ListStyleRef ret = 0;
    ATEErr error = sListStyleSet->Item(fListStyleSet, nIndex, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}

ATETextDOM::Int32 IListStyleSet::Find( const IListStyle ListStyle)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyleSet->Find_AsListStyle(fListStyleSet, ListStyle.GetRef(), &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyleSet::Find( const ATETextDOM::Unicode* name)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyleSet->Find_AsUnicode(fListStyleSet, name, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyleSet::Find( ListStylePredefinedTag tag)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyleSet->Find_ByPredefinedListStyleTag(fListStyleSet, tag, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

bool IListStyleSet::Remove( ATETextDOM::Int32 nIndex)
{
    ATEBool8 ret;
    ATEErr error = sListStyleSet->Remove(fListStyleSet, nIndex, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

ATETextDOM::Int32 IListStyleSet::Add( IListStyle ListStyle, bool keepPrivate)
{
    ATETextDOM::Int32 ret;
    ATEErr error = sListStyleSet->Add(fListStyleSet, ListStyle.GetRef(), keepPrivate, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

bool IListStyleSet::Replace( ATETextDOM::Int32  nIndex, IListStyle ListStyle)
{
    ATEBool8 ret;
    ATEErr error = sListStyleSet->Replace(fListStyleSet, nIndex, ListStyle.GetRef(), &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}
...
void IMojiKumi::GetEntryPriority( ATETextDOM::Int32 index, ATETextDOM::Int32 * priority) const
{
    ATEErr error = sMojiKumi->GetEntryPriority(fMojiKumi, index, priority);
    if(error) Throw_ATE_Exception(error);
}

void IMojiKumi::GetEntryNoFloat( ATETextDOM::Int32 index, ATEBool8 * noFloat) const
{
    ATEErr error = sMojiKumi->GetEntryNoFloat(fMojiKumi, index, noFloat);
    if(error) Throw_ATE_Exception(error);
}
...
void IMojiKumi::SetEntryPriority( ATETextDOM::Int32 index, ATETextDOM::Int32 priority)
{
    ATEErr error = sMojiKumi->SetEntryPriority(fMojiKumi, index, priority);
    if(error) Throw_ATE_Exception(error);
}

void IMojiKumi::SetEntryNoFloat( ATETextDOM::Int32 index, ATEBool8 noFloat)
{
    ATEErr error = sMojiKumi->SetEntryNoFloat(fMojiKumi, index, noFloat);
    if(error) Throw_ATE_Exception(error);
}
...
FrameAlignment ITextFrame::GetFrameAlignment( ) const
{
    FrameAlignment ret;
    ATEErr error = sTextFrame->GetFrameAlignment(fTextFrame, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

void ITextFrame::SetFrameAlignment( FrameAlignment frameAlignment)
{
    ATEErr error = sTextFrame->SetFrameAlignment(fTextFrame, frameAlignment);
    if(error) Throw_ATE_Exception(error);
}
...
IListStyleSet IApplicationTextResources::GetListStyleSet( ) const
{
    ListStyleSetRef ret = 0;
    ATEErr error = sApplicationTextResources->GetListStyleSet(fApplicationTextResources, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyleSet(ret);
}
...
IListStyleSet IDocumentTextResources::GetListStyleSet( ) const
{
    ListStyleSetRef ret = 0;
    ATEErr error = sDocumentTextResources->GetListStyleSet(fDocumentTextResources, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyleSet(ret);
}
...
IArrayListStyleRef::IArrayListStyleRef()
:fArrayListStyleRef(0)
{
}

IArrayListStyleRef::IArrayListStyleRef(ArrayListStyleRefRef arrayListStyleref)
:fArrayListStyleRef(arrayListStyleref)
{
}

IArrayListStyleRef::IArrayListStyleRef(const IArrayListStyleRef& src)
:fArrayListStyleRef(src.fArrayListStyleRef)
{
...
IArrayListStyleRef::~IArrayListStyleRef()
{
    sArrayListStyleRef->Release(fArrayListStyleRef);
}

IArrayListStyleRef& IArrayListStyleRef::operator=(const IArrayListStyleRef& rhs)
{
    if (&rhs != this)
    {
        sArrayListStyleRef->Release(fArrayListStyleRef);
        fArrayListStyleRef = rhs.fArrayListStyleRef;
        sArrayListStyleRef->AddRef(fArrayListStyleRef);
    }
    return *this;
}

bool IArrayListStyleRef::operator==(const IArrayListStyleRef& rhs) const
{
    return fArrayListStyleRef == rhs.fArrayListStyleRef;
}
...
ArrayListStyleRefRef IArrayListStyleRef::GetRef() const
{
    return fArrayListStyleRef;
}

bool IArrayListStyleRef::IsNull() const
{
    return sArrayListStyleRef->IsNull(fArrayListStyleRef);
}

ATETextDOM::Int32 IArrayListStyleRef::GetSize( ) const
{
    ATETextDOM::Int32 ret;
    ATEErr error = sArrayListStyleRef->GetSize(fArrayListStyleRef, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

IListStyle IArrayListStyleRef::GetFirst( ) const
{
    ListStyleRef ret = 0;
    ATEErr error = sArrayListStyleRef->GetFirst(fArrayListStyleRef, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}

IListStyle IArrayListStyleRef::GetLast( ) const
{
    ListStyleRef ret = 0;
    ATEErr error = sArrayListStyleRef->GetLast(fArrayListStyleRef, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}

IListStyle IArrayListStyleRef::Item( ATETextDOM::Int32 index) const
{
    ListStyleRef ret = 0;
    ATEErr error = sArrayListStyleRef->Item(fArrayListStyleRef, index, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyle(ret);
}
...
IArrayListStyleSetRef::IArrayListStyleSetRef()
:fArrayListStyleSetRef(0)
{
}

IArrayListStyleSetRef::IArrayListStyleSetRef(ArrayListStyleSetRefRef arrayListStylesetref)
:fArrayListStyleSetRef(arrayListStylesetref)
{
}

IArrayListStyleSetRef::IArrayListStyleSetRef(const IArrayListStyleSetRef& src)
:fArrayListStyleSetRef(src.fArrayListStyleSetRef)
{
    sArrayListStyleSetRef->AddRef(fArrayListStyleSetRef);
}

IArrayListStyleSetRef::~IArrayListStyleSetRef()
{
    sArrayListStyleSetRef->Release(fArrayListStyleSetRef);
}

IArrayListStyleSetRef& IArrayListStyleSetRef::operator=(const IArrayListStyleSetRef& rhs)
{
    if (&rhs != this)
    {
        sArrayListStyleSetRef->Release(fArrayListStyleSetRef);
        fArrayListStyleSetRef = rhs.fArrayListStyleSetRef;
        sArrayListStyleSetRef->AddRef(fArrayListStyleSetRef);
    }
    return *this;
}

bool IArrayListStyleSetRef::operator==(const IArrayListStyleSetRef& rhs) const
{
    return fArrayListStyleSetRef == rhs.fArrayListStyleSetRef;
}
...
ArrayListStyleSetRefRef IArrayListStyleSetRef::GetRef() const
{
    return fArrayListStyleSetRef;
}

bool IArrayListStyleSetRef::IsNull() const
{
    return sArrayListStyleSetRef->IsNull(fArrayListStyleSetRef);
}

ATETextDOM::Int32 IArrayListStyleSetRef::GetSize( ) const
{
    ATETextDOM::Int32 ret;
    ATEErr error = sArrayListStyleSetRef->GetSize(fArrayListStyleSetRef, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

IListStyleSet IArrayListStyleSetRef::GetFirst( ) const
{
    ListStyleSetRef ret = 0;
    ATEErr error = sArrayListStyleSetRef->GetFirst(fArrayListStyleSetRef, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyleSet(ret);
}

IListStyleSet IArrayListStyleSetRef::GetLast( ) const
{
    ListStyleSetRef ret = 0;
    ATEErr error = sArrayListStyleSetRef->GetLast(fArrayListStyleSetRef, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyleSet(ret);
}

IListStyleSet IArrayListStyleSetRef::Item( ATETextDOM::Int32 index) const
{
    ListStyleSetRef ret = 0;
    ATEErr error = sArrayListStyleSetRef->Item(fArrayListStyleSetRef, index, &ret);
    if(error) Throw_ATE_Exception(error);
    return IListStyleSet(ret);
}
...
ATETextDOM::Real ICharFeatures::GetTCYUpDownRealAdjustment( bool* isAssigned) const
{
    ATETextDOM::Real ret;
    ATETextDOM::Float tempRet;
    ATEBool8 isAssignedRet;
    ATEErr error = sCharFeatures->GetTCYUpDownRealAdjustment(fCharFeatures, &isAssignedRet, &tempRet);
    if(error) Throw_ATE_Exception(error);
    if (isAssigned) *isAssigned = isAssignedRet;
    ret = tempRet;
    return ret;
...
ATETextDOM::Real ICharFeatures::GetTCYLeftRightRealAdjustment( bool* isAssigned) const
{
    ATETextDOM::Real ret;
    ATETextDOM::Float tempRet;
    ATEBool8 isAssignedRet;
    ATEErr error = sCharFeatures->GetTCYLeftRightRealAdjustment(fCharFeatures, &isAssignedRet, &tempRet);
    if(error) Throw_ATE_Exception(error);
    if (isAssigned) *isAssigned = isAssignedRet;
    ret = tempRet;
    return ret;
...
void ICharFeatures::SetTCYUpDownRealAdjustment( ATETextDOM::Real newVal)
{
    ATEErr error = sCharFeatures->SetTCYUpDownRealAdjustment(fCharFeatures, static_cast<ATETextDOM::Float>(newVal));
    if(error) Throw_ATE_Exception(error);
}

void ICharFeatures::SetTCYLeftRightRealAdjustment( ATETextDOM::Real newVal)
{
    ATEErr error = sCharFeatures->SetTCYLeftRightRealAdjustment(fCharFeatures, static_cast<ATETextDOM::Float>(newVal));
    if(error) Throw_ATE_Exception(error);
}
...
IArrayReal ICharInspector::GetTCYUpDownRealAdjustment( ) const
{
    ArrayRealRef ret = 0;
    ATEErr error = sCharInspector->GetTCYUpDownRealAdjustment(fCharInspector, &ret);
    if(error) Throw_ATE_Exception(error);
    return IArrayReal(ret);
}

IArrayReal ICharInspector::GetTCYLeftRightRealAdjustment( ) const
{
    ArrayRealRef ret = 0;
    ATEErr error = sCharInspector->GetTCYLeftRightRealAdjustment(fCharInspector, &ret);
    if(error) Throw_ATE_Exception(error);
    return IArrayReal(ret);
}
...
ATETextDOM::Int32 IParaFeatures::GetListTier(bool *isAssigned) const
{
    ATETextDOM::Int32 ret;
    ATEBool8 isAssignedRet;
    ATEErr error = sParaFeatures->GetListTier(fParaFeatures, &isAssignedRet, &ret);
    if(error) Throw_ATE_Exception(error);
    if (isAssigned) *isAssigned = isAssignedRet;
    return ret;
}
...
IListStyle IParaFeatures::GetListStyle( bool* isAssigned) const
{
    ListStyleRef ret = 0;
    ATEBool8 isAssignedRet;
    ATEErr error = sParaFeatures->GetListStyle(fParaFeatures, &isAssignedRet, &ret);
    if(error) Throw_ATE_Exception(error);
    if (isAssigned) *isAssigned = isAssignedRet;
    return IListStyle(ret);
}
...
void IParaFeatures::SetListStyle( IListStyle newVal)
{
    ATEErr error = sParaFeatures->SetListStyle(fParaFeatures, newVal.GetRef());
    if(error) Throw_ATE_Exception(error);
}

void IParaFeatures::SetListTier( ATETextDOM::Int32 newVal)
{
    ATEErr error = sParaFeatures->SetListTier(fParaFeatures, newVal);
    if(error) Throw_ATE_Exception(error);
}
...
void IParaFeatures::ClearListStyle( )
{
    ATEErr error = sParaFeatures->ClearListStyle(fParaFeatures);
    if(error) Throw_ATE_Exception(error);
}
...
IArrayInteger IParaInspector::GetListTier() const
{
    ArrayIntegerRef ret = 0;
    ATEErr error = sParaInspector->GetListTier(fParaInspector, &ret);
    if(error) Throw_ATE_Exception(error);
    return IArrayInteger(ret);
}
...
IArrayListStyleRef IParaInspector::GetListStyle( ) const
{
    ArrayListStyleRefRef ret = 0;
    ATEErr error = sParaInspector->GetListStyle(fParaInspector, &ret);
    if(error) Throw_ATE_Exception(error);
    return IArrayListStyleRef(ret);
}
...
void ISpell::SetSearchRange( const ITextRange& pRange, const bool resetEndOfSentence, const bool resetCurPos, const bool includeWordsOnSearchBoundary)
{
    ATEErr error = sSpell->SetSearchRange(fSpell, pRange.GetRef(), resetEndOfSentence, resetCurPos, includeWordsOnSearchBoundary);
    if(error) Throw_ATE_Exception(error);
}
...
bool ISpell::FindOneMisspelledWord( SpellCheckingResult* pResult, ITextRange pRange, Language* pLanguage, bool populateSuggestionList )
{
    ATEBool8 ret;
    ATEErr error = sSpell->FindOneMisspelledWord(fSpell, pResult, pRange.GetRef(), pLanguage, populateSuggestionList, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}
...
{
    ATEBool8 ret;
    ATEErr error = sSpell->WordExistsInUserDictionary(fSpell, pWord, checkEquivalentFormsSetting, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}

bool ISpell::WordExistsInIgnoreDictionary( const ATETextDOM::Unicode* pWord, const ATE::CheckEquivalentFormsSetting checkEquivalentFormsSetting)
{
    ATEBool8 ret;
    ATEErr error = sSpell->WordExistsInIgnoreDictionary(fSpell, pWord, checkEquivalentFormsSetting, &ret);
    if(error) Throw_ATE_Exception(error);
    return ret;
}
...
#endif //SLO_ENABLE_LILO_LINGUISTIC
...
void ITextRange::ConvertListStyleToText()
{
    ATEErr error = sTextRange->ConvertListStyleToText(fTextRange);
    if(error) Throw_ATE_Exception(error);
}
...
void ITextRanges::ConvertListStyleToText()
{
    ATEErr error = sTextRanges->ConvertListStyleToText(fTextRanges);
    if(error) Throw_ATE_Exception(error);
}
