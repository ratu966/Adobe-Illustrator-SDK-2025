...
#define kAIAnnotatorDrawerSuiteVersion9		AIAPI_VERSION(9)
#define kAIAnnotatorDrawerSuiteVersion		kAIAnnotatorDrawerSuiteVersion9
...
struct AIAnnotatorDrawerSuite
{
	...
	*/
	...
	/**  Draws an Image from pixel buffer in an annotation.
	@param inDrawer The annotation drawer object.
	@param inImageData The raw image pixel data. Bit-depth of components must be 8.
	@param inWidth The width of the image.
	@param inWidth The height of the image.
	@param inColorSpace The color model of the pixel data, an \c #AIRasterColorSpace constant. Only supported values are kRGBColorSpace and kAlphaRGBColorSpace.
	@param inUnscaled If true, drawing is done without scaling the image to document dpi.
	@param inReferencePoint The reference point at which to draw the image.
	@param inReferencePointIsCenter If true, reference point is considered as center. Otherwise reference point is considered as top-left.
	*/
	AIAPI AIErr (*DrawImage) (AIAnnotatorDrawer *inDrawer,
								const ai::uint8 *inImageData,
								ai::uint32 inWidth,
								ai::uint32 inHeight,
								ai::int16 inColorSpace,
								AIBoolean inUnscaled,
								const AIPoint &inReferencePoint,
								AIBoolean inReferencePointIsCenter);
