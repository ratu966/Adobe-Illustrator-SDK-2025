...
/**
	For frequent access to the dictionary using the AIDictionarySuite::Key()
	method, generating a key every time can be costly as it results in a
	search in the string pool.

	Thess set of macros are provided to avoid paying that cost for each access.
	...
	Alternatively, if the key is only required in a source file, declare and
	define with:
	...
	You need to specify array of key string (with or without name) as given in
	the	following code:
