...
typedef struct
{
	...
	/** Reports whether the current selection is valid for ReColor.
		(Note that this function returns a boolean value, not an error code.)
			@return True if selection valid, false otherwise.
		*/
	AIAPI AIBoolean (*IsSelectionValidforRecolor) ( void );
    
    /**
     * Reduces the number of colors in given art
     * @param inArt input art
     * @param numColors num of colors required in given art
     */
    AIAPI AIErr (*ReduceNumberOfColors) (AIArtHandle inArt, const ai::uint32& numColors);
