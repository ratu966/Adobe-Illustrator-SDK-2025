...
#define kAIDataFilterSuiteVersion11		AIAPI_VERSION(11)

/** Data filter suite version. */
#define kAIDataFilterSuiteVersion		kAIDataFilterSuiteVersion11
...
namespace ai
{
	...
	{
		/** Default compression level specifier for zstd. */
		constexpr int32 kAIZstdDefaultCompressionLevel{ 0 };

		/** Default base exponent value for growable buffer: 16 MB. */
		constexpr uint8 kAIGrowableBufferDefaultBaseExp{ 24 };
		/** Default increment exponent value for growable buffer: 4 times. */
		constexpr uint8 kAIGrowableBufferDefaultIncExp{ 2 };
	}
}
...
struct AIDataFilterSuite {
	...
	/** Creates a new random access data filter that writes to and reads from a list of
		growable memory buffers, which behave as a single unified buffer.
		Buffers are created starting with initial size indicated by \c baseExp and each
		subsequent buffer's size is doubled till the limit indicated by \c incExp is reached.

		Use to create a temporary memory storage to write out and then read in variable
		sized data.

			@param baseExp 2^baseExp is the size of the first buffer allocated.
				Can be 0, in which case size of first buffer allocated will be 1 byte.
			@param incExp 2^(baseExp+incExp) is the largest size of any buffer allocated.
				Can be 0, in which case size of all buffers will be same as the first buffer.
			@param filter [out] A buffer in which to return the new filter reference.

			@note Value of \c baseExp + \c incExp should be less than or equal to 32.
		*/
	AIAPI AIErr (*NewGrowableBufferDataFilter) ( ai::uint8 baseExp, ai::uint8 incExp, AIDataFilter** filter );
	...
	/** Creates a new Zstandard data filter that compresses or decompresses data from a linked filter.
			...
			@param filter [out] A buffer in which to return the new filter reference.
		*/
	AIAPI AIErr (*NewZStdDataFilter) ( const char *statestr, ai::int32 compressionLevel, AIDataFilter **filter );
