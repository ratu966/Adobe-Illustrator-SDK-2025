...
enum AIDrawArtFlagBitPos {
    ...
    */
    kAIDrawArtShadow                            = 28,
	/** When set, draw intertwine arts as image 
	*/
	kIntertwineArtAsImage						= 29,
	/** When set, draw intertwine arts as image
	*/
	kIntertwineArtWithPreciseZone				= 30,
    
    /** When set, text characters over which any COLR font is applied
        will be outlined while exporting to PDF.
    */
    kAIDrawCOLRTextAsOutlineForPDFExport        = 31
};
...
enum AIDrawArtFlagsValue {
	...
	kAIDrawIntertwineArtAsImageMask				= (1 << kIntertwineArtAsImage),
	kAIDrawIntertwineArtWithPreciseZone			= (1 << kIntertwineArtWithPreciseZone),
    kAIDrawCOLRTextAsOutlineForPDFExportMask    = (1 << kAIDrawCOLRTextAsOutlineForPDFExport),
};
...
typedef struct AIDrawArtData {
	...
	/** ObjectInfo collector */
	ObjUpdateInfoCollector* collector {nullptr};
public:
