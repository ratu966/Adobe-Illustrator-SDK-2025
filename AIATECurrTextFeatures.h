...
#define kAIATECurrentTextFeaturesSuiteVersion4	AIAPI_VERSION(4)
#define kAIATECurrentTextFeaturesSuiteVersion	kAIATECurrentTextFeaturesSuiteVersion4
...
typedef struct {
	...
	/**
		For a Character attribute, if there is an assigned overriden value in document, then this value is returned, else default value set
		in the parent style sheet is returned.
			@param pFeatures [out] A buffer in which to return the features.
	 */
	AIAPI AIErr (*GetFlattenedCharFeatures) ( ATE::CharFeaturesRef *pFeatures );
	
	/**
	 For a Paragraph attribute, if there is an assigned overriden value in document, then this value is returned, else default value set
			in the parent style sheet is returned.
			@param pFeatures [out] A buffer in which to return the features.
	 */
	
	AIAPI AIErr (*GetFlattenedParaFeatures) ( ATE::ParaFeaturesRef *pFeatures );
